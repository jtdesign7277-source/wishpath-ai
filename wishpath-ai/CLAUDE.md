# CLAUDE.md — WishPath.ai Current State
...
| AI Coach | Claude Opus 4 with web search tools |
| AI Roadmap | Claude Sonnet 4 |
...
**$4.99 Roadmap (working):** `index.html` → `wishpath-checkout` → Stripe → `wishpath-webhook` → Claude Sonnet generates roadmap → saves to `wishpath_orders` → emails via Resend → `success.html` polls and renders.
